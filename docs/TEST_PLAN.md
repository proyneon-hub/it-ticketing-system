...
| SLA escalation: once per ticket, fixed clock, safe to rerun and to run at the same time                               | `slaEscalation.test.ts`, `sla.integration.test.ts`, `tests/smoke-live/jobs.spec.ts`                                                                                            |
| Outbox: atomic with the change, retries, dead events, no double sends, no comment text                                | `outbox.test.ts`, `outbox.integration.test.ts` (stub webhook, forced failures, concurrent workers), `cronAuth.test.ts`                                                         |
| Metrics: who may read them, bounded route labels, counters, and dashboard and Prometheus config in step with the code | `metrics.test.ts`, `metrics.integration.test.ts`, `observability.test.ts`, the `observability` job in `ci.yml` (boots Prometheus and Grafana)                                  |
| Incident issues from the hourly health check: severity, wording, recovery                                             | `test_monitor.py`, the `support-ops-health-check.yml` workflow                                                                                                                 |
| Accessibility                                                                                                         | `tests/accessibility/` and the manual checklist in [ACCESSIBILITY_TESTING.md](ACCESSIBILITY_TESTING.md)                                                                        |
| Production image and the whole stack                                                                                  | The real-stack job in `.github/workflows/ci.yml`                                                                                                                               |
