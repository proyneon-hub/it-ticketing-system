...
| Outbox: atomic with the change, retries, dead events, no double sends, no free text                                   | `outbox.test.ts`, `outbox.integration.test.ts` (stub webhook, forced failures, concurrent workers), `cronAuth.test.ts`                                                         |
