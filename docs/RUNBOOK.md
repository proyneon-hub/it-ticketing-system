...
| `AGENT_DEFAULT_MODE`                  | No                | Where the agent starts before its settings are changed: `off`, `shadow` (default), `assist` or `auto`. Where auto is not available (Vercel, unless `AGENT_ALLOW_AUTO=true`), `auto` behaves as `assist`                                     |
...
| `AGENT_ALLOW_AUTO`                    | No                | Set to exactly `true` to allow auto mode on Vercel. Without it auto mode is only available locally and in Docker, and a setting of `auto` runs as assist                                                                                    |
| `AGENT_BREAKER_FAILURES`              | No                | Failed runs in a row that pause the agent, default 5                                                                                                                                                                                        |
| `AGENT_BREAKER_COOLDOWN_MS`           | No                | How long it stays paused after the latest failure, default 600000 (ten minutes)                                                                                                                                                             |
...
| `AGENT_API_RATE_LIMIT_MAX`            | No                | Requests allowed per caller per window on the agent's routes (settings, runs, handover, answers) and the decisions on its proposals, default 120. Counted per user, or per agent run                                                        |
| `AGENT_API_IP_RATE_LIMIT_MAX`         | No                | The same routes, per address, before sign-in is checked, default 600. Only stops floods                                                                                                                                                     |
| `AGENT_API_RATE_LIMIT_WINDOW_MS`      | No                | Their window, default 1 minute                                                                                                                                                                                                              |
...
The agent reads each new ticket, searches the knowledge base, sets the category, priority and group, and either drafts a reply for a person to approve or hands the ticket to a person. What it may do depends on the mode: in **shadow** it only records what it would do; in **assist** it triages and hands over for real, and its drafted reply waits for a technician to approve, edit or reject it; in **auto** it may also answer a ticket alone, for the categories you list, where the deployment allows it (below). Decisions: [ADR 009](adr/009-agent-as-api-client.md), [ADR 010](adr/010-agent-worker-and-outbox-consumer.md), [ADR 011](adr/011-agent-rollout-shadow-assist-auto.md), [ADR 012](adr/012-server-side-citation-enforcement.md).
...
**Auto mode (the agent answering alone).** Turn it on last, and for as little as possible. Set the default mode to `auto`, list on the Agent page the categories it may answer alone (start with one), and leave Security off the list: the server refuses Security whatever the list says. Before doing it, run the evaluation and look at citation validity for that category ([docs/EVAL_HISTORY.md](EVAL_HISTORY.md)); nothing else checks that the steps in a reply follow from the articles it cites. A reply the agent posted alone is labelled _AI-generated · not reviewed_, and its ticket shows `posted`. To stop it, flip the kill switch (it also stops the endpoint) or take the category off the list. Auto is unavailable on Vercel unless `AGENT_ALLOW_AUTO=true`, and the Agent page says so; there a setting of `auto` runs as assist.

**The circuit breaker.** After five failed runs in a row the agent pauses itself for ten minutes and new tickets go straight to people (runs are recorded as `circuit_open`); then one run is tried, and the breaker closes if it works. The Agent page shows an alert while it is open, `agent_circuit_open` is 1, and the settings API has a `circuit` field. Look at the failing runs (Agent page, outcome `error`) for the reason: the model provider, the key, or something in the agent's own code. Tickets it held are not retried; they are already with people.

**Limits that send a ticket to a person.** The daily cost cap, the per-requester hourly limit (default 5 runs an hour, set on the admin page) and a person editing the ticket while the agent works each end the run without the agent touching the ticket further. The run's reason says which: `daily_cost_cap`, `requester_rate_limited`, `ticket_changed`, `circuit_open`.
