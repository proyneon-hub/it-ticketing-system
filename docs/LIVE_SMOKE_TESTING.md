...
| `session.spec.ts`          | A real browser stays signed in after a reload through the httpOnly refresh cookie; refresh tokens rotate and a used one is refused; only admins reach `/api/users` and `/api/audit` |
