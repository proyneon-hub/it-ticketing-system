...
Without Docker, `npm run dev:db` starts a throwaway MongoDB, and `npm run build` followed by `NODE_ENV=production AUTH_SECRET=any-value node server.js` serves the production build the same way the image does.
