...
import { StatsGrid, WorkflowStrip } from './StatsGrid';
import TicketFilters from './TicketFilters';
import { demoUsers, emptyStats } from '../test/fixtures';
...
describe('Pagination', () => {
  it('disables Previous on the first page and Next on the last', () => {
    const { rerender } = render(
      <Pagination
        pagination={{ page: 1, limit: 10, totalPages: 3, total: 25 }}
        loading={false}
        onPage={vi.fn()}
      />
    );
    ...
    rerender(
      <Pagination
        pagination={{ page: 3, limit: 10, totalPages: 3, total: 25 }}
        loading={false}
        onPage={vi.fn()}
      />
    );
  ...
  it('asks for the neighbouring page and locks while loading', async () => {
    ...
    const { rerender } = render(
      <Pagination
        pagination={{ page: 2, limit: 10, totalPages: 3, total: 25 }}
        loading={false}
        onPage={onPage}
      />
    );
...
describe('stats', () => {
  it('prefers server stats, and falls back to what is visible while they load', () => {
    const { rerender } = render(
      <StatsGrid stats={undefined} activeCount={4} breachedVisibleCount={2} />
    );
    ...
    rerender(
      <StatsGrid
        stats={{ ...emptyStats, total: 12, sla: { breached: 5, dueSoon: 3 } }}
        activeCount={4}
  ...
  it('lists every workflow status with its count, defaulting to zero', () => {
    render(<WorkflowStrip stats={{ ...emptyStats, byStatus: { open: 7 } }} />);
