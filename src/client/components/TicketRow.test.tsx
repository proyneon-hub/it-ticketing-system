...
function renderRow({ ticket = makeTicket(), role = 'admin', expanded = false }: RowOptions = {}) {
  const props = {
...
describe('editing', () => {
  it('patches the status and priority the user picks', async () => {
    ...
    const { onPatch } = renderRow({ ticket });
    ...
    expect(onPatch).toHaveBeenNthCalledWith(1, ticket, { status: 'in-progress' });
    expect(onPatch).toHaveBeenNthCalledWith(2, ticket, { priority: 'urgent' });
  });
  ...
  it('saves an assignee only when it actually changed', async () => {
    ...
    await user.tab();
    expect(onPatch).toHaveBeenCalledWith(ticket, { assignee: 'Theo Technician' });
  });
...
describe('display', () => {
  it.each<[string, number, Ticket['status']]>([
    ['breached', -2, 'open'],
    ['due-soon', 6, 'open'],
    ['healthy', 60, 'open'],
    ['met', -2, 'resolved'],
  ...
  it('shows the activity timeline only when expanded', () => {
    ...
    const { unmount } = renderRow({ ticket, expanded: false });
