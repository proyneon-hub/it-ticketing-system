import { activityLabel, formatDate, getSlaState, label } from './format.js';

const HOUR = 60 * 60 * 1000;
const NOW = new Date('2026-06-01T12:00:00Z').getTime();
const due = (offsetHours) => new Date(NOW + offsetHours * HOUR).toISOString();
...
describe('getSlaState', () => {
  ...
  it.each(['resolved', 'closed'])('is met for %s tickets even when overdue', (status) => {
    expect(getSlaState({ status, dueAt: due(-100) }, NOW)).toBe('met');
  });

  it('is met when the ticket has no due date', () => {
    expect(getSlaState({ status: 'open' }, NOW)).toBe('met');
  });
});
